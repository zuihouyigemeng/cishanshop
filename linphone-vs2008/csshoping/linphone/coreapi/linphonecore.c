...
static SalMediaDescription *create_local_media_description(LinphoneCore *lc, const char *localip, const char *username, bool_t only_one_codec)
{
	MSList *l;
	PayloadType *pt;
...
LinphoneCall * linphone_call_new_outgoing(struct _LinphoneCore *lc, LinphoneAddress *from, LinphoneAddress *to)
{
	...
	call->localdesc=create_local_media_description (lc,call->localip,linphone_address_get_username(from),FALSE);
	...
	for (i=0;get_codec(lc->config,"audio_codec",i,&pt);i++){
		if (pt){
			if (!ms_filter_codec_supported(pt->mime_type)){
				...
				audio_codecs=ms_list_append(audio_codecs,pt);
		}
	}
...
void linphone_core_iterate(LinphoneCore *lc){
	...
	if (lc->call!=NULL){
		...
		if (call->dir==LinphoneCallIncoming && call->state==LCStateRinging){
			...
			if(lc->audiostream!=NULL) 
				audio_stream_iterate(lc->audiostream);
#ifdef VIDEO_ENABLED
			if (lc->videostream!=NULL)
...
__declspec(dllexport) void test_version(void)
{
	printf("hello linphone \n");
}